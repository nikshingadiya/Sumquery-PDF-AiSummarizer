...
def extract_information(file_path):
    ...
    extra_info_matches = re.findall(extra_info_pattern, script_text)
    ...
    for scene_match in scenes:
        ...
        if scene_match.group(5) is not None:
            ...
            transitions_pattern = re.compile(r'^\s*(CUT TO:|FADE TO BLACK:|DISSOLVE TO:|FADE (?:TO BLACK|IN):)\s*$', re.MULTILINE)
            ...
            scene_data["Extensions"] = extensions.group(2) if extensions else None

        extracted_data["Scenes"].append(scene_data)
        current_scene_data = scene_data

    # Add extra information to the scene data
    for extra_info in extra_info_matches:
        if current_scene_data:
            current_scene_data["Extra Information"].append(extra_info.strip())
